...
%%-------
% modification to original code by Lars-Christian Tokle
% laser angles and distance extraction
AAr = (0:360)*pi/360; % laser angles

% range (and validity?) extraction
Mask13 = uint16(2^13 - 1);
MaskA  = bitcmp(Mask13);

RR = double(bitand( Mask13,LASER))/100; % distance
a  = uint16(bitand( MaskA ,LASER)); % some check?
%%--------
...
iib= ceil(auxi) ;
Rs = (R1(iia)+R1(iib))/2 ;
x = [ Rs+dL5/2;(A5+A5u)*0.5 - pi/2]; %;dL5;] ; % distance , angle, size ;
